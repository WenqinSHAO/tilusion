...
entities, locations, atoms, time cues, unresolved narrative threads, and evidence.
...
- Check whether a new atom continues, advances, or resolves a known thread. Reference the prior thread in `context_alignment_notes` when confident.
...
- `atom_mentions`: local compositional atoms — self-contained pieces that play a role in the book. Every atom has a kind and belongs to at least one thread. Only atoms with temporal attributes carry time references.
...
Minimum JSON shape:
{
  ...
  "atom_mentions": [
    {
      ...
      "summary": "brief grounded description of this compositional piece",
  ...
  "context_alignment_notes": [
    {
      "local_record_id": "atom-0001",
      "local_record_type": "atom_mention",
...
- When a local atom continues, advances, or resolves a prior thread from `book_scope_context.context.active_threads`, note the relationship in `context_alignment_notes`. Reference the prior thread id and explain the narrative signal briefly.
...
- New entities, locations, atoms, and threads may appear that have no prior record. Extract them as normal local mentions when supported by `text`.
- Every entity mention, location mention, atom mention, time expression, and thread candidate must cite `evidence_span_ids`.
...
- IDs are temporary and response-local only. Use stable IDs within this response such as `evidence-0001`, `entity-0001`, `location-0001`, `atom-0001`, `time-0001`, and `thread-0001`.
- `atom_kind` classifies the nature of the compositional piece: `narrative_event` for temporally-grounded occurrences, `technique` for methods or how-to descriptions, `observation` for descriptive notes, `habit` for recurring behaviors or preferences, `argument` for opinions or claims, `other` when none fit.
- `time_expression_ids` may be `[]` or `null` for atoms without temporal attributes (e.g. techniques, habits, observations).
- Every atom must reference at least one `thread_id` from `thread_candidates` in this response. Each thread captures a thematic or narrative line that the atom participates in.
